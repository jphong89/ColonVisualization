...
vtkSmartPointer<vtkPolyData> Centerline::EliminateTorsion(RenderManager* t_rendermanager, vtkSmartPointer<vtkPolyData> t_colon)
{
    ...
    Directions->Reset();
    ViolationNums->Reset();
    colors->Reset();colors->SetNumberOfComponents(3);colors->SetName("Colors");
    ...
    */
    ...
    std::cout<<"Iteration Ends: "<<iter<<" "<<model->GetNumberOfPoints()<<endl;
    }
    ...
    CurvaturePointsActor->GetProperty()->SetPointSize(6);
    ...
    ViolationPointsActor->GetProperty()->SetPointSize(2);
